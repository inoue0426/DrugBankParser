...
def get_target_name(target, ns):
    return target.find('db:name', ns).text if target.find('db:name', ns) is not None else 'N/A'
...
def parse_articles(target, ns, res, drug_name, drugbank_id, **kwargs):
    ...
    pmids = [
        ...
        if article.find('db:pubmed-id', ns) is not None and article.find('db:pubmed-id', ns).text is not None
    ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse DrugBank XML file and save target information to CSV')
    parser.add_argument('--input', default='drugbank.xml', help='Path to the DrugBank XML file to parse')
    parser.add_argument('--output', default='targets.csv', help='Path to the output CSV file')
